    ...
    public partial class FOV : Form
    {
        ...
        private void Fov_timer_Tick(object sender, EventArgs e)
        {
            if(COD.Process_Handle("iw3mp"))
            {
                ...
                COD.WriteFloat_Protected32(COD.ReadInteger32(Offsets.All_FPS_Offsets[0,1]) + Offsets.All_FPS_Offsets[0, 0], tb_Fov.Value);
            }
            else if (COD.Process_Handle("iw4mp"))
            {
                lbl_Game.Text = "Call Of Duty Modern-Warfare 2 MP";
                COD.WriteFloat_Protected32(COD.ReadInteger32(Offsets.All_FPS_Offsets[1, 1]) + Offsets.All_FPS_Offsets[1, 0], tb_Fov.Value);
            }
            else if (COD.Process_Handle("iw5mp"))
            {
                lbl_Game.Text = "Call Of Duty Modern-Warfare 3 MP";
                COD.WriteFloat_Protected32(COD.ReadInteger32(Offsets.All_FPS_Offsets[2, 1]) + Offsets.All_FPS_Offsets[2, 0], tb_Fov.Value);
            }
            ...
            else if (COD.Process_Handle("t6mp"))
            {
                ...
                COD.WriteFloat_Protected32(COD.ReadInteger32(Offsets.All_FPS_Offsets[3, 1]) + Offsets.All_FPS_Offsets[3, 0], tb_Fov.Value);
            }
            else
            {
                lbl_Game.Text = "No game detected";
            }
        }
        #endregion
    }
